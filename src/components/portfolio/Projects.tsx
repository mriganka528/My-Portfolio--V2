...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-28 px-8 md:px-20 border-t relative" style={{ borderColor: "rgba(0,229,184,0.08)" }}>
      <div className="max-w-7xl mx-auto">
          ...
          {data.projects.map((p) => {
            ...
            return (
              <div key={p.id} className="proj-row" data-hover>
                <button type="button" aria-expanded={isOpen} aria-controls={`project-${p.id}`} onClick={() => setOpen(isOpen ? null : p.id)} className="project-heading w-[calc(100%+2rem)] text-left flex items-center gap-6 py-5 -mx-4 px-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-baseline gap-4">
                      <span className="font-head font-bold transition-colors duration-200"
                  ...
                  <div className="flex items-center gap-6 shrink-0">
                    ...
                    <span className="font-code text-[10px]" style={{ color: "var(--dim)" }}>{p.year}</span>
                ...
                <motion.div id={`project-${p.id}`} inert={!isOpen} aria-hidden={!isOpen} initial={false} animate={{ height: isOpen ? "auto" : 0, opacity: isOpen ? 1 : 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                  <div className="project-details pb-8 pl-20 pr-4 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-8 items-start">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {p.stack.map((t, i) => (
                          <span key={`${t}-${i}`} className="font-code text-[10px] px-2.5 py-1 border"
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between mt-8 pt-6 border-t" style={{ borderColor: "rgba(0,229,184,0.08)" }}>
          ...
          {externalUrl(data.profile.github) && <a href={externalUrl(data.profile.github)} target="_blank" rel="noopener noreferrer" className="font-code text-xs hover:underline" style={{ color: "var(--teal)" }}>
            {data.profile.github} →
          </a>}
        </div>
      </div>
